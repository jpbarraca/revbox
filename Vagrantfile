Vagrant.configure("2") do |config|
 ...
 config.vm.provider "virtualbox" do |v|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    apt-get update
    apt install -y ghidra binutils ltrace gdb-multiarch  python3-pip docker.io apktool edb-debugger jd-gui jdax wireshark qemu-user qemu-system imhex pulseview zapproxy 
